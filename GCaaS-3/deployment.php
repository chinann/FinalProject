    ...
    <div class="tab-content">
        ...
        <div class="tab-pane active" id="control-sidebar-home-tab">
            ...
            <div class="input-group" style="margin: 10px">
                <button class="btn btn-warning btn-block" type="button" id="search-btn" onclick="drop()"><i class="fa fa-map-marker"></i> Add Marker</button>
            </div><!-- /input-group -->
...
<!--                    checkbox hospital-->
                    <div class="checkbox" id="check-hos">
                        ...
                        <div id="hospital-list" class="collapse" style="padding-left: 20px">
                            <label><input type="checkbox" id="h1" onclick="checkSomeHos()">Government School</label><br>
                            <label><input type="checkbox" id="h2" onclick="checkSomeHos()">Private School</label><br>
                    ...
                    <div class="checkbox" id="check-sch">
                        ...
                        <div id="school-list" class="collapse" style="padding-left: 20px">
                    ...
                    <div class="checkbox">
                        ...
                        <div id="temple-list" class="collapse" style="padding-left: 20px">
            ...
            <form method="post">
                ...
                <div class="input-group">
                    ...
                    <div class="collapse" id="twitter-menu">
                        ...
                        <div class="input-group">
                            <input type="text" class="form-control"  id="hashShow" placeholder="" style="margin-bottom: 10px;width: inherit" disabled>
                        </div>
                        <div class="input-group">
                            <input type="text" class="form-control" id="add-hash" placeholder="Ex.#floodTH ...">
                            <span class="input-group-btn">
                                <button class="btn btn-default" type="button" id="ok" onclick="change()"><i class="fa fa-check"
                                                                                         aria-hidden="true"></i>
                                </button>
                                <script>
                                    var hashtag = "";
                                    var newHashtag = "";
                                    function change(){
//                                        document.getElementById('hashShow').value = document.getElementById('add-hash').value;
                                        newHashtag += document.getElementById('add-hash').value;
                                        document.getElementById('hashShow').value = newHashtag;
                    ...
                    // tw = setInterval(function(){ 
                        ...
                        for (var i = 0; i < myLatlng.length; i++) {
                            if (myLatlng[i].status == "New"){
                                ...
                                '<div id="siteNotice">'+
                                '</div>'+
                                '<h3 id="firstHeading" class="firstHeading">' + '<img src =\"' + myLatlng[i].profileImg + '\">' + "&nbsp&nbsp&nbsp" + myLatlng[i].name +'</h3>'+
                                '<div id="bodyContent">'+
                                '<p><b>Message </b>: '+ myLatlng[i].msg +'</p>'+
                                '<p><b>Date of tweets </b>: '+ myLatlng[i].date +'</p>'+
                                '<p><b>Status </b>: <select id="status">'+
                        ...
                        // markerTWCluster = new MarkerClusterer(map, markerTWClus);
                    // },5000);
                }
            }
          ...
          if (xmlhttp.readyState==4 && xmlhttp.status==200 ) //200 OK do 4 stage
            {
              ...
              if (myLatlng.data.length==0) {
                ...
                for (var i = 0; i < myLatlng.data.length; i++) {
                  var contentStr = '<div id="content">'+
                  '<div id="siteNotice">'+
                  '</div>'+
                  '<h1 id="firstHeading" class="firstHeading">'+ myLatlng.data[i].name +'</h1>'+
                  '<div id="bodyContent">'+
                  '<p><b>Definition:</b></p>'+
                  '<p><b>Reference:</b></p>'+
                  '<p>ละติจูด: '+ myLatlng.data[i].y +' ลองจิจูด: '+ myLatlng.data[i].x +'</p>'+
                  '</div>'+
                  '<p>ลบสถานที่, <input type="button" id="info" onclick=deleteInfoXMLDoc('+ i+') value="'+  myLatlng.data[i].name  +'">'+
                  '</p>'+
                  '</div>';
                  if (i == myLatlng.data.length-1) {
                    var latlng = new google.maps.LatLng(myLatlng.data[i].y,myLatlng.data[i].x);
                    marker[i] = new google.maps.Marker({
                      position: latlng,
                      map: map,
                      title: myLatlng.data[i].name,
                      info: new google.maps.InfoWindow({
                       content: contentStr
                      })
                    });

                    infowindow = marker[i].info;
                    ...
                    marker[i] = new google.maps.Marker({
                      position: new google.maps.LatLng(myLatlng.data[i].y,myLatlng.data[i].x),
                      map: map,
                      title: myLatlng.data[i].name,
                      info: new google.maps.InfoWindow({
                      content: contentStr
