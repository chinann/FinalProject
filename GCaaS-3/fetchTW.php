...
<?php
    ...
    $connect = pg_connect("host=localhost port=5432 dbname=DB_". $_SESSION['depname'] ." user=postgres password=1234");
    if (!$connect) {
        ...
        foreach ($tweets as $tweet){
            foreach($tweet as $t){
                ...
                if ($rows == 0) {
                    if (isset($t->geo)) {
                        $sql = "INSERT INTO \"table_postTWH\"(\"post_Name\", \"post_GeomIncident\", \"post_Date\", \"post_Status\", \"post_Message\", \"post_Hashtag\", \"post_ProfileImg\", \"post_Place\", \"deployment_Name\") VALUES ('" . $t->user->name ."', ST_GeomFromText('POINT(" . $t->geo->coordinates[1] . " " . $t->geo->coordinates[0] . ")',4326), '" . $t->created_at . "', 'New' , '" . $t->text . "', 'GCaaS', '" . $t->user->profile_image_url . "', '" . $t->place->full_name . "', '" . $_SESSION['depname'] . "');";
                        echo $sql;
